...
async def _transcribe(file: UploadFile, language: str | None, transcriber) -> TranscriptionResult:
    ...
    try:
        ...
        with TemporaryDirectory(prefix='hackalem-audio-') as directory:
            ...
            result = await run_in_threadpool(transcriber.transcribe, path, language=language)
...
async def transcribe(file: Annotated[UploadFile, File()],
    ...
    return await _transcribe(file, language, transcriber)
...
async def analyze_audio(file: Annotated[UploadFile, File()],
                        ...
                        transcriber=Depends(get_transcriber)):
    result = await _transcribe(file, language, transcriber)
    request = AnalyzeRequest(title=title, transcript=result.transcript)
    protocol = await run_in_threadpool(_analyze, request)
    return AudioAnalysisResponse(protocol=protocol, language=result.language,
