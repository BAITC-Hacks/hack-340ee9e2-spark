...
// Frontend and API are served by the same local FastAPI process.
...
async function request(path, options = {}, asBlob = false) {
  ...
  const timeoutMs = path.split("?")[0] === API.analyzeAudio ? API.analysisTimeoutMs : asBlob ? API.exportTimeoutMs : API.requestTimeoutMs;
...
function setBusy(busy) {
  state.busy = busy;
  ["audio-file", "remove-file", "process-button", "audio-limit"].forEach((id) => { $(id).disabled = busy; });
...
function renderResults(result) {
  ...
  state.diarizationAvailable = result.diarization_available && result.diarization?.status === "ok";
  ...
  const reasons = {
    speaker_change_within_segment: "Внутри реплики сменился говорящий",
    overlapping_speech: "Одновременная речь нескольких говорящих",
    insufficient_coverage: "Недостаточно данных для определения говорящего",
    no_matching_speech: "Нет совпадающего интервала речи",
  };
  state.transcript.forEach((segment, index) => {
    ...
    if (!segment.speaker) {
      const assignment = result.diarization?.assignments?.find((item) => item.segment_index === index);
      content.append(node("small", "processing-note", reasons[assignment?.reason] || "Говорящий не определён; проверьте запись."));
    }
    row.append(node("span", "timestamp", `${timestamp(segment.start)}–${timestamp(segment.end)}`), content);
...
async function processMeeting(event) {
  ...
  resetResults();
  setBusy(true);
  setStatus("processing", "Распознаём речь, определяем говорящих и составляем протокол. Дождитесь результата; повторная отправка отключена.");
  try {
    const form = new FormData();
    form.append("file", state.file);
    const limit = $("audio-limit").value;
    const path = API.analyzeAudio + (limit === "60" ? "?max_seconds=60" : "");
    const result = await request(path, {method: "POST", body: form});
    connection(true);
    renderResults(result);
    setBusy(false);
    const interval = result.processing;
    const note = interval ? `Обработан интервал ${interval.processed_start_seconds}–${interval.processed_end_seconds} с.${interval.is_partial ? " Это только начало записи." : ""}` : "Результат получен.";
    setStatus("completed", note, 100);
  } catch (error) {
    setBusy(false);
